        ...
        if train_data_X.columns[j] != "ID":   # 处理除了ID的其他列数据
            ...
            for i in range(len_):
                ...
                if (array[0] > std_max) | (array[0] < std_min):
                    ...
                    #   array[0] = mean_del
                    array[0] = mean_
                if (array[len_ - 1] > std_max) | (array[len_ - 1] < std_min):  # 第一个数据和最后一个数据单独处理
                    array[len_ - 1] = mean_
                if i > 0 and i < (len_ - 1):
                    if (array[i] > std_max) | (array[i] < std_min):
                        if (array[i - 1] < std_max) and (array[i - 1] > std_min) and (array[i + 1] < std_max) and (
                                array[i + 1] > std_min):
                            array[i] = np.float(array[i - 1] + array[i + 1]) / 2
                        elif ((array[i - 1] < std_max) and (array[i - 1] > std_min)) | (array[i + 1] > std_max) | (
                                array[i + 1] < std_min):
                            array[i] = array[i - 1]
                        elif (array[i - 1] < std_max) | (array[i - 1] > std_min) | (
                                (array[i + 1] < std_max) and (array[i + 1] > std_min)):
                            array[i] = array[i + 1]
...
def generate_train_data(train_data, test_data, poly=False, select=False):
    y = train_data['发电量']
    X = train_data.drop(['发电量', 'ID'], axis=1)  # 删除列
    sub_data = test_data.drop(['ID'], axis=1)

    polynm = None
    if poly:
        from sklearn.preprocessing import PolynomialFeatures  # PolynomialFeatures多项式特征
        polynm = PolynomialFeatures(degree=2, interaction_only=True)
        X = polynm.fit_transform(X)
        sub_data = polynm.transform(sub_data)

    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=123)  # 分割训练集和测试集

    sm = None
    if select:
        from sklearn.feature_selection import SelectFromModel  # 特征选择
        sm = SelectFromModel(GradientBoostingRegressor(random_state=2))  # random_state作为每次产生随机数的随机种子
...
# 分数计算
def cal_score(mse):
    if isinstance(mse, float):  # isinstance判断一个对象是否是一个已知的类型 考虑继承关系
...
def cross_validation_test(models, train_X_data, train_y_data, cv=5):
    ...
    for i, model in enumerate(models):
        ...
        nmse = cross_val_score(model, train_X_data[i], train_y_data[i], cv=cv, scoring='neg_mean_squared_error')  # 交叉验证
    ...
    for i in np.arange(len(array)):
        for j in np.arange(10):
            ...
            if i < 10:
                ...
                num = (array[i - 1] + array[i - 2] + array[i - 3] + array[i - 4] + array[i - 5] + array[i - 6] + array[
                    i - 7] + array[i - 8] + array[i - 9]) / 9
        newarray.append(num)
    df["old平均功率"] = newarray
    return df
...
db_info = {
    'user': 'root',
    'password': 'zxm740103QIAN',
    'host': 'localhost',
    'database': 'elel'  # 事先指定了数据库，后续操作只需要表即可
}
...
# 读取数据
sql_1 = "train"
train_data = pd.read_sql(sql_1, conn)  # 返回值是一个 pandas 的数据类型（DataFrame、Series）

sql_2 = "test"
test_data = pd.read_sql(sql_2, conn)
...
for idx, line in bad_data.iterrows():
    ...
    replace_value = (all_data.loc[index - before_offset, col_index].values + all_data.loc[
        index + after_offset, col_index].values) / 2
...
# 保存数据
df_result.to_sql('result', con=conn, index=False, if_exists='append')